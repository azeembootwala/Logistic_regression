...
class LogisticModel(object):
    ...
    def fit_model(self,X,T,lr =10e-4,reg=10e-1,epoch=2800,show_fig=False):
        N,D = X.shape
        ...
        for i in range(0,epoch):
            ...
            c=self.cross_entropy(T,Y_pred)

            self.W = self.W - lr*(X.T.dot(Y_pred-T)+reg*np.sign(self.W))

            if i%40 ==0:
                r = self.classification_rate(Y_pred,T)
                cost.append(c)
